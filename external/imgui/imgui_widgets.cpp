...
bool ImGui::ButtonCtEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    {
        bgCol = ImColor(0.18f, 0.42f, 0.44f, 1.00f);
        borderCol = ImColor(0.30f, 0.78f, 0.74f, 0.70f);
    }
    else if (hovered)
    {
        bgCol = ImColor(0.22f, 0.30f, 0.36f, 1.00f);
        borderCol = ImColor(0.30f, 0.60f, 0.58f, 0.50f);
    }
    else
    {
        bgCol = ImColor(0.16f, 0.18f, 0.23f, 1.00f);
        borderCol = ImColor(0.24f, 0.28f, 0.35f, 0.50f);
    }

    window->DrawList->AddRectFilled(bb.Min, bb.Max, bgCol, rounding);
    window->DrawList->AddRect(bb.Min, bb.Max, borderCol, rounding, 0, 1.2f);

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");

    RenderTextClipped(bb.Min + style.FrameButtonPaddingCt, bb.Max - style.FrameButtonPaddingCt, label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::ImageButtonExCt(ImGuiID id, ImTextureID texture_id, ImGuiButtonFlags flags, const ImVec2& size, const ImVec2 &imgp, const ImVec2 &imgs, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
  ...
  if (!(flags & ImGuiButtonFlags_NoBackground))
  {
    ...
    {
      bgCol = ImColor(0.14f, 0.16f, 0.20f, 1.00f);
      borderCol = ImColor(0.22f, 0.25f, 0.32f, 0.50f);
    }

    window->DrawList->AddRectFilled(bb.Min, bb.Max, bgCol, rounding);
    window->DrawList->AddRect(bb.Min, bb.Max, borderCol, rounding, 0, 1.0f);
  }
...
bool ImGui::CheckboxCt(const char* label, bool* v)
{
  ...
  ImU32 bgCol = (v && *v)
    ? ImColor(0.20f, 0.52f, 0.50f, 1.00f)
    : (hovered ? ImColor(0.18f, 0.20f, 0.26f, 1.00f) : ImColor(0.14f, 0.16f, 0.20f, 1.00f));
  ImU32 borderCol = (v && *v)
    ? ImColor(0.30f, 0.78f, 0.74f, 0.80f)
    : (hovered ? ImColor(0.30f, 0.60f, 0.58f, 0.50f) : ImColor(0.24f, 0.28f, 0.35f, 0.50f));

  window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, bgCol, rounding);
  window->DrawList->AddRect(check_bb.Min, check_bb.Max, borderCol, rounding, 0, 1.2f);
...
bool ImGui::BeginComboCt(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
  ...
  const float rounding = style.FrameRounding;
  ImU32 bgCol = hovered ? ImColor(0.18f, 0.20f, 0.26f, 1.00f) : ImColor(0.14f, 0.16f, 0.20f, 1.00f);
  ImU32 borderCol = hovered ? ImColor(0.30f, 0.60f, 0.58f, 0.50f) : ImColor(0.24f, 0.28f, 0.35f, 0.50f);
...
bool ImGui::SliderScalarCt(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
  ...
  const float rounding = style.FrameRounding;
  ImU32 trackCol = ImColor(0.14f, 0.16f, 0.20f, 1.00f);
  ImU32 borderCol = ImColor(0.22f, 0.25f, 0.32f, 0.50f);

  window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, trackCol, rounding);
  window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, borderCol, rounding, 0, 1.0f);
  ...
  if (grab_bb.Max.x > grab_bb.Min.x)
  {
    const ImGuiCol grabCol = (g.ActiveId == id ? ImGuiCol_SliderCtGrabActive : ImGuiCol_SliderCtGrab);
    const float grabRounding = style.GrabRounding;
    window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(grabCol), grabRounding);
  }
